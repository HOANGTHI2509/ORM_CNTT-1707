...
class NhanVien(models.Model):
    ...
    @api.model
    def cron_chuc_mung_sinh_nhat(self):
        """ Gửi thông báo SMS/Zalo chúc mừng sinh nhật nhân viên có ngày sinh trong ngày hôm nay. """
        today = date.today()
        # Tìm tất cả nhân viên có ngày và tháng sinh nhật trùng với hôm nay
        employees = self.search([])
        for emp in employees:
            if emp.ngay_sinh and emp.ngay_sinh.day == today.day and emp.ngay_sinh.month == today.month:
                message = f"🎉 Chúc mừng sinh nhật {emp.ho_va_ten}! Chúc bạn có một ngày thật vui vẻ và hạnh phúc."
                if emp.so_dien_thoai:
                    self.env['zalo.notification'].send_message(emp.so_dien_thoai, message)
