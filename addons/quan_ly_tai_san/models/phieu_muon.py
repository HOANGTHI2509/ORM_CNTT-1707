...
class PhieuMuon(models.Model):
    ...
    def action_approve(self):
        for record in self:
            ...
            if record.state == 'draft':
                ...
                # Gửi thông báo Zalo khi phiếu mượn được duyệt
                msg = f"✅ Phiếu mượn tài sản {record.ma_phieu_muon} của bạn đã được duyệt. Vui lòng nhận tài sản theo thời gian dự kiến: {record.ngay_muon_du_kien.strftime('%H:%M %d/%m/%Y')}."
                if record.nhan_vien_id and record.nhan_vien_id.so_dien_thoai:
                    self.env['zalo.notification'].send_message(record.nhan_vien_id.so_dien_thoai, msg)
    ...
    def cron_canh_bao_qua_han(self):
        ...
        template = self.env.ref('quan_ly_tai_san.email_template_canh_bao_qua_han', raise_if_not_found=False)
        for slip in overdue_slips:
            if slip.nhan_vien_id:
                # Gửi mail nếu nhân viên có email
                if template and slip.nhan_vien_id.work_email:
                    template.send_mail(slip.id, force_send=True)
                
                # Gửi thông tin nhắc nhở qua Zalo
                msg = f"⏰ Cảnh báo quá hạn: Bạn có phiếu mượn '{slip.ma_phieu_muon}' đã quá hạn trả ({slip.ngay_tra_du_kien.strftime('%H:%M %d/%m/%Y')}). Vui lòng hoàn tất trả tài sản."
                if slip.nhan_vien_id.so_dien_thoai:
                    self.env['zalo.notification'].send_message(slip.nhan_vien_id.so_dien_thoai, msg)
