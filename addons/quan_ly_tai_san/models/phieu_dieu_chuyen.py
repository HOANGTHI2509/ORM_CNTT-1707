...
class PhieuDieuChuyen(models.Model):
    ...
    def action_duyet(self):
        ...
        # Thông báo cho người quản lý tài sản hoặc người đang dùng (nếu có)
        msg = f"✅ Phiếu điều chuyển tài sản '{self.tai_san.ten_tai_san}' (Mã: {self.ten_phieu}) từ {self.vi_tri_hien_tai.ten_vi_tri} sang {self.vi_tri_moi.ten_vi_tri} đã được duyệt."
        if self.tai_san.quan_ly_id:
            self._gui_thong_bao_zalo(self.tai_san.quan_ly_id, msg)
        elif self.tai_san.nguoi_dang_dung_id:
            self._gui_thong_bao_zalo(self.tai_san.nguoi_dang_dung_id, msg)
